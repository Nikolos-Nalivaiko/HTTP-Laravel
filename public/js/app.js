(() => {
    ...
    (() => {
        ...
        function openDropdown() {
            const menuItems = document.querySelectorAll(".menu__item p[data-target]");
            menuItems.forEach((item => {
                item.addEventListener("click", (() => {
                    const currentlyActiveItem = document.querySelector(".menu__item.active");
                    const targetDropdown = document.getElementById(item.getAttribute("data-target"));
                    if (currentlyActiveItem && currentlyActiveItem !== item.parentElement) {
                        currentlyActiveItem.classList.remove("active");
                        const activeDropdown = document.getElementById(currentlyActiveItem.querySelector("p[data-target]").getAttribute("data-target"));
                        if (activeDropdown) activeDropdown.classList.remove("active");
                    }
                    item.parentElement.classList.toggle("active");
                    if (targetDropdown) targetDropdown.classList.toggle("active");
                }));
            }));
        }
        function openFilter() {
        ...
        if ($("#region-load-select").val()) selectCities_loadCities("#region-load-select", "#city-load-select");
        if ($("#region-unload-select").val()) selectCities_loadCities("#region-unload-select", "#city-unload-select");
        if ($("#region_id").val()) selectCities_loadCities("#region_id", "#city_id");
        window["FLS"] = true;
        addLoadedClass();
        menuInit();
    })();
